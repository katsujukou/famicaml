...
(** 最小限の合法 iNES ヘッダを生成する。
    prg_banks: PRG バンク数 (×16KB)
    chr_banks: CHR バンク数 (×8KB)、0 = CHR-RAM
    flags6: mirroring=V なら 0x01、battery なら 0x02、trainer なら 0x04 *)
...
let test_nrom_2prg_1chr () =
  (* NROM: PRG 2バンク(32KB), CHR 1バンク(8KB) *)
...
let test_trainer_present () =
  ...
  let (Cart.NROM { prg; _ }) = cart.Cart.rom in
  Alcotest.(check int) "PRG size" 16384 (Bytes.length prg)

let test_prg_data_preserved () =
  (* PRG-ROM の先頭バイトが正しくコピーされているか確認 *)
  let data = make_rom ~prg_banks:1 ~chr_banks:0 () in
  Bytes.set_uint8 data 16 0xEA;   (* NOP at PRG offset 0 *)
  Bytes.set_uint8 data 17 0xA9;   (* LDA *)
  let cart = ok_or_fail (Ines.parse data) in
  let (Cart.NROM { prg; _ }) = cart.Cart.rom in
...
let test_error_unsupported_mapper () =
  let data = make_rom ~flags6:0x20 ~prg_banks:1 ~chr_banks:0 () in  (* mapper 2 *)
  match Ines.parse data with
  | Error (Ines.Unsupported_mapper 2) -> ()
  | _ -> Alcotest.fail "expected Unsupported_mapper 2"
