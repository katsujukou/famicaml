...
let test_jmp_indirect_bug () =
  ...
  let ram, bus, cpu = make_env () in
  Bytes.set_uint8 ram 0x01FF 0x00;
  (* lo = $00 *)
  Bytes.set_uint8 ram 0x0100 0x03;
  (* hi (バグ読み) = $03 → $0300 *)
  Bytes.set_uint8 ram 0x0200 0xFF;
  (* hi (正常読み) = $FF → $FF00 *)
  wr ram 0x0300 [ 0xA9; 0x42 ];
  (* バグで到達するアドレス: LDA #$42 *)
  wr ram 0xFF00 [ 0xA9; 0xAA ];
  (* 正常動作なら到達するアドレス: LDA #$AA *)
  wr ram 0 [ 0x6C; 0xFF; 0x01 ];
  (* JMP ($01FF) *)
  run bus cpu 2;
...
let test_jsr_rts () =
  ...
  (* $0000: JSR $0100
     ...
     $0100: LDA #$FF
     $0102: RTS         *)
  wr ram 0x0000 [ 0x20; 0x00; 0x01 ];
  (* JSR $0100 *)
  wr ram 0x0003 [ 0xA9; 0x01 ];
  (* LDA #$01 *)
  wr ram 0x0100 [ 0xA9; 0xFF; 0x60 ];
  (* LDA #$FF ; RTS *)
  run bus cpu 4;
  (* JSR, LDA#$FF, RTS, LDA#$01 *)
  chk8 "A" 0x01 cpu.reg_A
...
let test_brk () =
  ...
  wr ram 0x0200 [ 0xA9; 0x77 ];
  (* IRQ ハンドラ: LDA #$77 *)
  wr ram 0 [ 0x00 ];
  (* BRK *)
  run ~ith_irq:0x0200 bus cpu 2;
  (* BRK + LDA#$77 *)
