...
let mk () =
  ...
  let bus_read_access p =
    ...
    if u <= memory_map.wram.to_ then
      Uint8.of_int (Bytes.get_uint8 wram (Uint16.to_int p land 0x07FF))
    else if u >= memory_map.prg_rom.from then
      Uint8.of_int ((current_mapper ()).read (Uint16.to_int p))
    else raise Exn.Out_of_range
  in
  let bus_write_access p x =
    let u = Uint16.to_int p in
    if u <= memory_map.wram.to_ then
      Bytes.set_uint8 wram (Uint16.to_int p land 0x07FF) (Uint8.to_int x)
    else if u >= memory_map.prg_rom.from then
      (current_mapper ()).write (Uint16.to_int p) (Uint8.to_int x)
    else raise Exn.Out_of_range
  in
...
let connect (nes : t) (data : bytes) =
  ...
  | Error e -> Error e
  | Ok cart -> connect_cartridge nes cart; Ok ()
