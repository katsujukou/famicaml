...
let run_adc bus cpu arg =
  ...
  let overflow =
    Uint8.logand (Uint8.logxor a result) (Uint8.logxor m result)
    |> fun x -> Uint8.logand x bit7 <> Uint8.zero
  in
  cpu.reg_A <- result;
  cpu.reg_P
  <- PS.set_flags
...
let run_sbc bus cpu arg =
  ...
  let result = u8 (sum land 0xFF) in
  let overflow =
    Uint8.logand (Uint8.logxor a result) (Uint8.logxor m_inv result)
    |> fun x -> Uint8.logand x bit7 <> Uint8.zero
  in
  cpu.reg_A <- result;
  cpu.reg_P
...
let run_inx cpu =
  cpu.reg_X <- Uint8.(cpu.reg_X + one);
  set_zn cpu cpu.reg_X

let run_iny cpu =
  cpu.reg_Y <- Uint8.(cpu.reg_Y + one);
  set_zn cpu cpu.reg_Y
...
let run_brk (bus : Bus.t) (cpu : Register.t) ~ith_irq =
  push16 bus cpu Uint16.(cpu.reg_PC + one);
  push bus cpu (Uint8.logor (PS.to_uint8 cpu.reg_P) (u8 0x30));
  (* B=1 で push *)
  set_flag cpu I true;
  cpu.reg_PC <- ith_irq
